...
case class PagedResults[D <: Document[D], V](query: Query[D, V],
  ...
  def docs(implicit transaction: Transaction[D]): IO[List[D]] = ids.map(query.collection(_)).sequence

  def values(implicit transaction: Transaction[D]): IO[List[V]] = docs.flatMap(_.map(query.convert).sequence)
  ...
  def docStream(implicit transaction: Transaction[D]): fs2.Stream[IO, D] = idStream
    .evalMap { id =>
      getter match {
        case Some(g) => g(id)
  ...
  def stream(implicit transaction: Transaction[D]): fs2.Stream[IO, V] = docStream.evalMap(query.convert)

  def scoredStream(implicit transaction: Transaction[D]): fs2.Stream[IO, (V, Double)] = idAndScoreStream
    .evalMap {
