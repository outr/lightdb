...
import lightdb.traversal.pipeline.{Accumulators, Pipeline, Stages}
  ...
  "RocksDBTraversalAggregationPipelineSpec" should {
    "support typed match -> project -> groupBy" in {
      val p = Pipeline.from(Stream.emits(List(1, 2, 3, 4, 5, 6)))
        ...
        .pipe(Stages.groupBy[(String, Double), String, Double, Double](_._1, Accumulators.sum(_._2)))
    ...
    "support unwind + sort + skip/limit" in {
      val p = Pipeline.from(Stream.emits(List(
        ...
        .pipe(Stages.unwind[(String, List[Int]), Int](_._2))
        .pipe(Stages.sortBy(identity))
        .pipe(Stages.skip(1))
        .pipe(Stages.limit(2))
    ...
    "support bounded sortByPage for pagination without full materialization" in {
      ...
      val p = Pipeline.from(Stream.emits(input))
        .pipe(Stages.sortByPage[Int, Int](identity, offset = 2, limit = 3))
    ...
    "support facetTop (top-N counts) as a streaming facet-like reducer" in {
      ...
      val p = Pipeline.from(Stream.emits(input))
        .pipe(Stages.facetTop[String, String](identity, limit = 3))
    ...
    "support reduce2(count, facetTop) to compute multiple reducer outputs in one pass" in {
      ...
      val p = Pipeline.from(Stream.emits(input))
        .pipe(Stages.reduce2(
          Accumulators.count[String],
          Accumulators.facetTop[String, String](identity, limit = 2)
        ))
    ...
    "support groupBy2 (two accumulators) and facet2" in {
      ...
      val grouped = base.pipe(Stages.groupBy2[String, String, Long, Long, Double, Double](
        key = identity,
        a1 = Accumulators.count,
        a2 = Accumulators.sum(_ => 1.0)
      ))

      val faceted = base.pipe(Stages.facet2(Stages.count, Stages.groupBy(identity, Accumulators.count)))
    ...
    "support groupBy3 (three accumulators)" in {
      ...
      val grouped = base.pipe(Stages.groupBy3[Int, String, Long, Long, Double, Double, Option[Int], Option[Int]](
        key = n => if (n % 2 == 0) "even" else "odd",
        a1 = Accumulators.count,
        a2 = Accumulators.sum(_.toDouble),
        a3 = Accumulators.max(identity)
      ))
