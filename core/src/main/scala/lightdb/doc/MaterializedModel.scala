...
trait MaterializedModel[Doc <: Document[Doc], MaterialDoc <: Document[MaterialDoc], MaterialModel <: DocumentModel[MaterialDoc]] extends DocumentModel[Doc] { mm =>
  ...
  protected def adding(doc: MaterialDoc)(implicit transaction: Transaction[MaterialDoc, MaterialModel]): Task[Unit]
  protected def modifying(oldDoc: MaterialDoc, newDoc: MaterialDoc)(implicit transaction: Transaction[MaterialDoc, MaterialModel]): Task[Unit]
  protected def removing(doc: MaterialDoc)(implicit transaction: Transaction[MaterialDoc, MaterialModel]): Task[Unit]
  ...
  override protected def init[Model <: DocumentModel[Doc]](store: Store[Doc, Model]): Task[Unit] = {
    super.initialize(store).map { _ =>
      materialStore.trigger += new BasicStoreTrigger[MaterialDoc, MaterialModel] {
        ...
        override protected def adding(doc: MaterialDoc)(implicit transaction: Transaction[MaterialDoc, MaterialModel]): Task[Unit] = mm.adding(doc)
        override protected def modifying(oldDoc: MaterialDoc, newDoc: MaterialDoc)(implicit transaction: Transaction[MaterialDoc, MaterialModel]): Task[Unit] = mm.modifying(oldDoc, newDoc)
        override protected def removing(doc: MaterialDoc)(implicit transaction: Transaction[MaterialDoc, MaterialModel]): Task[Unit] = mm.removing(doc)
