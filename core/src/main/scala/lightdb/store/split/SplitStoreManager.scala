...
case class SplitStoreManager(storage: StoreManager,
  ...
  override def create[Doc <: Document[Doc], Model <: DocumentModel[Doc]](db: LightDB,
    ...
    val storage = this.storage.create[Doc, Model](db, model, s"$name/storage", StoreMode.All())
    new SplitCollection(
      name = name,
      model = model,
      storage = storage,
      searching = searching.create[Doc, Model](db, model, s"$name/search", if (searchIndexAll) StoreMode.All() else StoreMode.Indexes(storage)),
      storeMode = storeMode,
      db = db,
      storeManager = this
    )
