...
//
//  protected def getWriteBuffer(transaction: Transaction[Doc, Model]): WriteBuffer[Doc]
//  protected def setWriteBuffer(writeBuffer: WriteBuffer[Doc], transaction: Transaction[Doc, Model]): Unit
//
//  protected def withWriteBuffer(f: WriteBuffer[Doc] => Task[WriteBuffer[Doc]])
//                               (implicit transaction: Transaction[Doc, Model]): Task[WriteBuffer[Doc]] = Task {
...
//                        (implicit transaction: Transaction[Doc, Model]): Task[WriteBuffer[Doc]] = withWriteBuffer { b =>
//    val s = b.map.size
//    f(b.map).map { map =>
//      val d = map.size - s
...
//  }
//
//  override def prepareTransaction(transaction: Transaction[Doc, Model]): Task[Unit] = Task {
...
//  }
//
//  override protected def _insert(doc: Doc)(implicit transaction: Transaction[Doc, Model]): Task[Doc] = writeMod { map =>
//    Task(map + (doc._id -> WriteOp.Insert(doc)))
//  }.map(_ => doc)
//
//  override protected def _upsert(doc: Doc)(implicit transaction: Transaction[Doc, Model]): Task[Doc] = writeMod { map =>
//    Task(map + (doc._id -> WriteOp.Upsert(doc)))
//  }.map(_ => doc)
//
//  protected def _exists(id: Id[Doc])(implicit transaction: Transaction[Doc, Model]): Task[Boolean]
//
//  override final def exists(id: Id[Doc])(implicit transaction: Transaction[Doc, Model]): Task[Boolean] = Task.defer {
//    val wb = getWriteBuffer(transaction)
//    wb.map.get(id) match {
...
//                                   (implicit transaction: Transaction[Doc, Model]): Task[Boolean] = if (index == idField) {
//    writeMod { map =>
//      Task {
...
//  }
//
//  protected def _truncate(implicit transaction: Transaction[Doc, Model]): Task[Int]
//
//  override final def truncate()(implicit transaction: Transaction[Doc, Model]): Task[Int] = Task.defer {
//    var count = 0
//    withWriteBuffer { wb =>
//      Task {
//        count = wb.map.size
