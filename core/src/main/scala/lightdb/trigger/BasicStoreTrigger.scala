...
trait BasicStoreTrigger[Doc <: Document[Doc], Model <: DocumentModel[Doc]] extends StoreTrigger[Doc, Model] {
  ...
  protected def modifying(oldDoc: Doc, newDoc: Doc)(implicit transaction: Transaction[Doc, _ <: Model]): Task[Unit]

  protected def removing(doc: Doc)(implicit transaction: Transaction[Doc, _ <: Model]): Task[Unit]

  override final def insert(doc: Doc)(implicit transaction: Transaction[Doc, _ <: Model]): Task[Unit] = adding(doc)

  override final def upsert(doc: Doc)(implicit transaction: Transaction[Doc, _ <: Model]): Task[Unit] =
