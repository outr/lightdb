...
To add all modules:
```scala
libraryDependencies += "com.outr" %% "lightdb-all" % "1.1.2"
```

For a specific implementation like Lucene:
```scala
libraryDependencies += "com.outr" %% "lightdb-lucene" % "1.1.2"
```
...
```scala
val db = new DB
// db: DB = repl.MdocSession$MdocApp$DB@5ea74f6b
db.init()
// res0: Boolean = true
```
...
//   nicknames = Set(),
//   friends = List(),
//   _id = Id(value = "KX7pnHxh7qet74BmcULK6XK9lkcxnTCc")
// )
...
// List(MaterializedAggregate({"ageMin": 21, "ageMax": 21, "ageAvg": 21.0, "ageSum": 189},repl.MdocSession$MdocApp$Person$@15d2fb24))
```
