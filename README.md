...
To add all modules:
```scala
libraryDependencies += "com.outr" %% "lightdb-all" % "4.6.1"
```

For a specific implementation like Lucene:
```scala
libraryDependencies += "com.outr" %% "lightdb-lucene" % "4.6.1"
```
...
//   nicknames = Set(),
//   friends = List(),
//   _id = StringId(value = "IDmTU51mzoBQCEyaxBuHrwtLEcmHTags")
// )
...
// People in their 20s: List(Person(Adam,21,None,Set(),List(),StringId(IDmTU51mzoBQCEyaxBuHrwtLEcmHTags)))
```
...
// Results: List(MaterializedAggregate({"ageMin": 21, "ageMax": 21, "ageAvg": 21.0, "ageSum": 21},repl.MdocSession$MdocApp$Person$@4bec2af2))
```
...
// Grouped: List(Grouped(21,List(Person(Adam,21,None,Set(),List(),StringId(IDmTU51mzoBQCEyaxBuHrwtLEcmHTags)))))
```
