...
To add all modules:
```scala
libraryDependencies += "com.outr" %% "lightdb-all" % "1.1.1"
```

For a specific implementation like Lucene:
```scala
libraryDependencies += "com.outr" %% "lightdb-lucene" % "1.1.1"
```
...
```scala
val db = new DB
// db: DB = repl.MdocSession$MdocApp$DB@2453bb8e
db.init()
// res0: Boolean = true
```
...
//   nicknames = Set(),
//   friends = List(),
//   _id = Id(value = "mF9hZfb8wM4meAcQeUKfAdZ75dELpE81")
// )
...
// List(MaterializedAggregate({"ageMin": 21, "ageMax": 21, "ageAvg": 21.0, "ageSum": 168},repl.MdocSession$MdocApp$Person$@2aa4a9f7))
```
