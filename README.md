...
| [Lucene](https://lucene.apache.org)                                   | Full-Text Search   | ✅          | ✅✅      | ✅        | ✅           | ❌              | ✅✅✅           | ✅ (HNSW)  | ❌        | Best-in-class full-text search engine; native block-join, spatial, and vector KNN |
...
| Store                                                                 | Type               | Persistence | Read Perf | Write Perf | Concurrency | Transactions    | Full-Text Search | Vector KNN | Prefix Scan | Notes                                 |
|------------------------------------------------------------------------|--------------------|-------------|-----------|------------|-------------|------------------|------------------|------------|-----------|---------------------------------------|
| [PostgreSQL](https://www.postgresql.org)                              | Relational DB      | ✅          | ✅✅✅     | ✅✅      | ✅✅         | ✅✅✅ (ACID, MVCC) | ✅✅ (TSVector)  | ✅ (pgvector) | 🟡         | Full-featured RDBMS; native vector via the pgvector extension |
...
| [Qdrant](https://qdrant.tech)                                         | Vector DB          | ✅          | ✅✅✅     | ✅✅      | ✅✅         | ❌              | 🟡 (Payload match) | ✅✅✅      | ❌        | Purpose-built vector DB; native HNSW KNN + payload filtering. Vector-first: not a general store (no prefix scan / traversal) |
...
To add all modules:
```scala
libraryDependencies += "com.outr" %% "lightdb-all" % "4.47.1"
```

For a specific implementation like Lucene:
```scala
libraryDependencies += "com.outr" %% "lightdb-lucene" % "4.47.1"
```
...
```scala
libraryDependencies += "com.outr" %% "lightdb-tantivy" % "4.47.1"
```

For graph traversal utilities:
```scala
libraryDependencies += "com.outr" %% "lightdb-traversal" % "4.47.1"
```

For OpenSearch:
```scala
libraryDependencies += "com.outr" %% "lightdb-opensearch" % "4.47.1"
```
...
//   input = FlatMap(
//     input = Suspend(
//       f = lightdb.store.Store$TransactionBuilder$$Lambda/0x000000004c9f0dc0@779c9400,
...
//       )
//     ),
//     f = lightdb.store.Store$TransactionBuilder$$Lambda/0x000000004c9f27a0@1813caa0,
//     trace = SourcecodeTrace(
//       file = File(
//         "/home/mhicks/projects/open/lightdb/core/src/main/scala/lightdb/store/Store.scala"
//       ),
...
//     )
//   ),
//   f = lightdb.store.Store$TransactionBuilder$$Lambda/0x000000004c9f3340@4754920a,
//   trace = SourcecodeTrace(
//     file = File(
//       "/home/mhicks/projects/open/lightdb/core/src/main/scala/lightdb/store/Store.scala"
//     ),
...
//   nicknames = Set(),
//   friends = List(),
//   _id = StringId("SE2ibqY5EO4WixN4xBT69kDnIyfxUuYZ")
// )
...
// Results: List(MaterializedAggregate({"ageMin": 21, "ageMax": 21, "ageAvg": 21.0, "ageSum": 1428},repl.MdocSession$MdocApp$Person$@624ea5d4,Map()))
```
...
//   _id = StringId("IPQMxIE6dRfQmulqCk6K7wD8uf6z4bHl")
// )
...
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("eRVoB53ej3WVcaw3hBG1HwiUayodFcUR")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("pYwrdhmIhD7fPGppaZL8abTHZljCsQ5w")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("JXBMWy3ZN1S7JG1Hnt2hQmRx6nJhc6eC")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("iG7tyAkJF3q0QlcODdVfOFjG3RUweDod")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("IFwzPeJnI1topAmWeKg0gcKMCvjqmiQ1")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("6k5Oc7iUEp9ElAsZKvNPUMAVgB8KFuQ2")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("cN1fXLLD1fbGUK9zOYZKKsY4aICOrzJ9")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("FZgV7bROXVln98WEp9QyKLBhfHmEwvU3")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("3lPkmGvmGdfXzkJROpQugBPWqcvobg9f")
//   ),
//   Note(
//     text = "the quick brown fox",
//     _id = StringId("lXkEJdTCoxl7HxpNIxWu0BKOvDrVOUGW")
//   ),
// ...
```
...
//   _id = StringId("JqEWbGFNm93l0Acq3HACBlwCk4OyXHAe")
// )
...
// tenantA: HashMapStore[TenantDoc, TenantDoc] = lightdb.store.hashmap.HashMapStore@651d0797
tenantA.transaction(_.insert(TenantDoc("hello"))).sync()
// res14: TenantDoc = TenantDoc(
//   value = "hello",
//   _id = StringId("AX9jp2Ad66GTqOtZUHMDEKOrUDGikgB5")
// )
```
...
//   key = "featureX",
//   store = lightdb.store.hashmap.HashMapStore@64d63c71,
//   default = repl.MdocSession$MdocApp$$Lambda/0x000000004cc19a50@43464004,
//   persistence = Stored
// )
