...
@EmbeddedTest
class TantivyParentChildSpec extends AbstractParentChildSpec {
  ...
  override protected def indexParentsAndChildren(parentsData: List[Parent], childrenData: List[Child]): Task[Unit] =
    DB.parents.transaction(_.insert(parentsData).unit).next(
      DB.children.transaction(_.insert(childrenData).unit)
    )

  object DB extends LightDB {
    override type SM = CollectionManager
    override val storeManager: CollectionManager = TantivyStore
