...
object AggregateFilter {
  ...
  case class Equals[F, D <: Document[D]](name: String, index: Index[F, D], value: F) extends AggregateFilter[D] {
    def getJson: Json = index.rw.read(value)
  }

  case class In[F, D <: Document[D]](name: String, index: Index[F, D], values: Seq[F]) extends AggregateFilter[D] {
  ...
  case class RangeLong[D <: Document[D]](name: String, index: Index[Long, D], from: Option[Long], to: Option[Long]) extends AggregateFilter[D]

  case class RangeDouble[D <: Document[D]](name: String, index: Index[Double, D], from: Option[Double], to: Option[Double]) extends AggregateFilter[D]

  case class Parsed[F, D <: Document[D]](name: String, index: Index[F, D], query: String, allowLeadingWildcard: Boolean) extends AggregateFilter[D]

  case class Distance[D <: Document[D]](name: String, index: Index[GeoPoint, D], from: GeoPoint, radius: Length) extends AggregateFilter[D]
}